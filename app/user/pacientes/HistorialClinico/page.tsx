...
import HistorialPaciente from "@/components/User/Pacientes/HistorialPaciente";
  ...
  const HandleGetPaciente = async (id: number) => {
    try {
      ...
      if (response.ok) {
        ...
        console.error("Error al obtener paciente:", data);
      }
    } catch (error) {
      console.error("Error de red al obtener paciente:", error);
    }
  };
  ...
  return (
    <div className="pb-8 bg-[#eaeded]">
      ...
      <div>
        ...
        >
          <HistorialPaciente idPaciente={Number(idPaciente)} />
        </div>
      </div>
    </div>
  );
